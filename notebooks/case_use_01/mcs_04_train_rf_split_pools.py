#!/usr/bin/env python3
"""
MCS Case Study — Script 4 (v3): Train RF over embedding split pools + schemas + predictions

Adds:
- prediction artifacts per split (train/val/test):
    - preds_train.csv / preds_val.csv / preds_test.csv
  containing:
    - index (original dataset index)
    - sequence
    - y_true
    - y_prob (P(y=1))
    - y_pred (thresholded at 0.5)

These predictions are useful for:
- fragility plots across seeds
- calibration curves / reliability diagrams
- error analysis & threshold sweeps
...
case_demo_01/
  ...
  splits/<strategy>/seed_<SEED>/
    ...
    embedding_<...>/train.parquet,val.parquet,test.parquet,schema.yaml

Outputs per (seed, embedding):
seed_<SEED>/models/random_forest/<embedding_id>/
  model.joblib
  metrics.json
...
def fit_best_rf(
    ...
    n_jobs: int,
) -> Tuple[RandomForestClassifier, Dict[str, object], Dict[str, float]]:
    best_params: Dict[str, object] = {}
    best_score = -1.0
    best_val_metrics: Dict[str, float] = {}

    for params in ParameterGrid(param_grid):
        clf = RandomForestClassifier(random_state=seed, n_jobs=n_jobs, **params)
    ...
    if not best_params:
        raise RuntimeError("Failed to select RF hyperparameters.")

    # Refit on train+val
    X_tv = np.vstack([X_train, X_val])
    y_tv = np.concatenate([y_train, y_val])

    final = RandomForestClassifier(random_state=seed, n_jobs=n_jobs, **best_params)
    final.fit(X_tv, y_tv)
    cleanup_memory()
...
def main() -> None:
    ...
    exec_schema = ExecutionSchema(
        ...
        packages=pip_freeze(),
        notes="RF training over embedding split pools. Predictions exported for fragility/calibration analysis.",
    )
    ...
    for i, emb_dir in enumerate(emb_dirs, start=1):
        print(f"[{i}/{len(emb_dirs)}] RF training for: {emb_dir}")
        ...
        model, best_params, best_val_metrics = fit_best_rf(
        ...
        prob_train = probs(X_train)
        prob_val = probs(X_val)
        prob_test = probs(X_test)
        ...
        # output dir
        out_dir = seed_dir / "models" / "random_forest" / embedding_id
        ...
        # save predictions (include original dataset indices + sequences)
        preds_train_path = out_dir / "preds_train.csv"
        preds_val_path = out_dir / "preds_val.csv"
        preds_test_path = out_dir / "preds_test.csv"

        # Ensure sequence column in preds corresponds to embedded rows (not raw order), so use emb_*_seq
